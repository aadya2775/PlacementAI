...
# load_dotenv()
# genai.configure(api_key=(os.getenv("e66a5fdab1191ab1729043d45bd7c6094704da27")))
...
# def overall(request):
#     global pairs
#     # skills = UserProfile.objects.filter(email = 'aadya2775@gmail.com').values_list('skills')
#     # lang = UserProfile.objects.filter(email = 'aadya2775@gmail.com').values_list('language')
#     role = 'Front End Developer'
#     dreamcompany = 'Amazon'
#     preferred_stack = 'HTML, CSS, JavaScript'
#     genai.configure(api_key=(os.getenv("e66a5fdab1191ab1729043d45bd7c6094704da27")))  # Set up your API key
...
def overall(request):
    ...
    convo = model.start_chat(history=[])
    context = ("Act as an advisor for engineering students by providing them a roadmap "
               "to achieve their dream role. Provide 7 phases with details for each.")
    convo.send_message(f"{context} Role: {role}, Dream Company: {dreamcompany}, Preferred stack: {preferred_stack}")
    result = convo.last.text
...
def category(request):
    ...
    convo = model.start_chat(history=[])
    context = ('You are an AI educator. Scrape the provided PDF link, and provide a response '
               'to the user question in 500 words with additional links and resources. If '
               'a flowchart is requested, provide one with an explanation.')
    convo.send_message(f"{context} Link: {url}, User Question: {user_input}")
    result = convo.last.text
...
def personal(request):
    ...
    convo = model.start_chat(history=[])
    context = (
        "Today's date is 23rd March 2024. Provide a personalized roadmap for students to "
        "achieve their dream job. Include weekly tasks based on the placement time, required "
        "skills, and current proficiency. Conclude with interview preparation."
    )
    convo.send_message(f"{context} Placement Time: {placement}, Current Skills: {proficiency}")
    result = convo.last.text

    weeks_tasks = re.findall(r'\*\*Week \d+:(.*?)\*\*Week \d+:|\Z', result, re.DOTALL)
    weeks_dict = {f"Week {i+1}": task.strip() for i, task in enumerate(weeks_tasks)}
...
def fullcalendar(request):
    ...
    data_json = dumps(events, cls=DjangoJSONEncoder)
    return render(request, 'roadmap/fullcalendar.html', {'events_json': data_json})
...
def hackathons(request):
    ...
    try:
        ...
        return render(request, 'pages/hackathons.html', {'error': f"Error fetching hackathons: {e}"})

    soup = BeautifulSoup(response.content, "html.parser")
    hackathons_data = []

    try:
        hackathon_links = soup.find_all("a", class_="hackathon-tile")
        for link in hackathon_links:
            title = link.find("h3").text.strip()
            href = link.get("href", "#")
            hackathons_data.append({"title": title, "link": href})
    except Exception as e:
        return render(request, 'pages/hackathons.html', {'error': f"Error parsing data: {e}"})
